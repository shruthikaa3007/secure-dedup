...
## Abstract
Cloud storage systems use deduplication to reduce storage and bandwidth by eliminating repeated data. However, deterministic deduplication can leak useful structure and can also allow attackers to claim duplicate ownership without possessing actual content. This project presents a secure chunk-level deduplication system centered on secret-assisted dedup tokens, fingerprint-bound segmented encryption, proof-of-ownership (PoW), behavioral anomaly detection, and adaptive policy control. The implemented system uses FastAPI-based APIs, HMAC- or hash-based chunk indexing, HKDF-derived AES-GCM chunk encryption, challenge-response verification for duplicate claims, and runtime risk-aware policy actions (`ALLOW`, `RATE_LIMIT`, `BLOCK`). Experimental validation compares a baseline public-hash encryption path against the proposed secret-assisted scheme while also measuring adaptive PoW effectiveness. The final prototype supports end-to-end demo workflows with reproducible scenario reports and runtime metrics.

## Chapter 1: Introduction
### 1.1 Background
Deduplication is essential for efficient cloud storage because many users store similar or identical data. Standard deduplication works by splitting a file into chunks, hashing each chunk, and storing only unique chunks. Although efficient, this approach introduces security risks when chunk identities are deterministic and duplicate checks are exposed to untrusted clients.
...
2. They must reduce deterministic leakage from deduplication identities and encryption behavior.
3. They must prevent malicious users from falsely claiming ownership of existing chunks.
4. They must handle repeated suspicious behavior in real time.

Therefore, the problem is to design and implement a system that preserves deduplication gains while improving dedup-aware encryption and adding practical ownership verification and abuse-resistance controls.
...
### 1.4 Scope
The work focuses on API-level secure deduplication and encryption-oriented defense mechanisms in a cloud-style storage setting. It does not claim production-scale benchmarking on very large distributed clusters.
...
4. For duplicate chunks: require PoW proof; reject if missing or invalid.
5. Extract behavior features and compute risk.
6. Decide policy (`ALLOW`, `RATE_LIMIT`, `BLOCK`) for subsequent requests.
7. Persist telemetry and runtime metrics.
...
2. Add encryption comparison between deterministic and secret-assisted schemes.
3. Add behavior feature extraction and detector integration.
4. Add policy and reputation control loop.
5. Build Swagger-guided and script-based scenario automation for validation.
...
3. Model artifacts loaded from trained supervised and unsupervised pipelines.
4. Automated scenario suite executes key attack and defense workflows.
5. Reports generated in JSON, Markdown, and CSV for reproducibility.

### 3.3 Evaluation Criteria
1. Functional correctness of upload, duplicate, PoW solve, and retry flow.
...
6. **UI and automation**: Swagger demo flow, Colab notebook, and scenario suite.

### 4.2 Key Endpoints
1. `/upload`, `/pow/challenge`, `/pow/verify`
2. `/demo/config`, `/metrics`, `/demo/status`
3. `/demo/encryption/comparison`
4. `/demo/compare-files`
5. `/demo/highlights/{client_id}`

## Chapter 5: Results and Discussion
### 5.1 Functional Validation
Recent automated validation confirms both workflow correctness and demo readiness:
1. Encryption unit tests passed.
2. Smoke validation passed with `7/7` checks.
3. Scenario execution passed with `8/8` end-to-end workflows.
4. Covered flows include baseline upload and duplicate detection.
5. Covered flows include PoW challenge, solve, and retry success.
6. Covered flows include policy force and recovery behavior.
7. Covered flows include encryption status checks and metrics snapshot.
8. Covered flows include secret-assisted dedup token mode exposure in the demo configuration and metrics pipeline.
...
### 5.3 Discussion
The results indicate that secure deduplication can remain operational while adding stronger dedup-token secrecy and active abuse controls. The proposed secret-assisted construction preserved dedup savings and storage overhead exactly in this benchmark, and its measurable cost was concentrated almost entirely in dedup-token generation rather than in bulk encryption or decryption. This is a favorable tradeoff for a security-first design because the additional cost stays at the fingerprinting boundary while the stored-chunk cryptographic path remains effectively unchanged.

## Chapter 6: Conclusion and Future Work
### 6.1 Conclusion
This project demonstrates a practical secure deduplication system that improves on deterministic dedup-encryption assumptions while preserving efficiency and preventing fake duplicate claims. The combined secret-assisted encryption + PoW + anomaly-aware policy architecture provides a defendable and implementable solution for an encryption-focused final-year project.

### 6.2 Future Work
1. Larger-scale benchmarking with higher concurrency and larger datasets.
2. Broader real-world attack trace evaluation.
3. Broader comparison against more randomized or server-aided dedup-encryption schemes.
4. Stronger chunk-sharing demonstrations on larger controlled corpora.

## References
1. J. Wu et al., "A randomized encryption deduplication method against frequency attack," *Journal of Information Security and Applications*, vol. 83, 2024, doi:10.1016/j.jisa.2024.103774.
...
4. X. Tang, C. Guo, K.-K. R. Choo, X. Jiang, Y. Liu, "A secure and lightweight cloud data deduplication scheme with efficient access control and key management," *Computer Communications*, vol. 222, 2024, pp. 209-219, doi:10.1016/j.comcom.2024.05.003.
5. C. Gan et al., "Coupling Secret Sharing with Decentralized Server-Aided Encryption in Encrypted Deduplication," *Applied Sciences*, vol. 15, no. 3, 2025, doi:10.3390/app15031245.
6. Project repository implementation and validation artifacts: `README.md`, `compare_dedup_encryption_schemes.py`, `tests/run_scenario_suite.py`, `docs/project_notes/encryption_scheme_comparison.md`, `advanced_artifacts/training_metrics.json`, `pow_comparison_summary.json`.
